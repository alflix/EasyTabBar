...
Available through [CocoaPods](http://cocoapods.org).
...
UITabBarController, 在 Apple 的交互规范中最常见的控件，几乎每个 iOS 开发都和它打过交道。虽然简单，但如何用好，如何实现常用的 UI 效果，以及如何处理多个 iOS 版本之间的适配，仍然需要花一些功夫。EasyTabBar 就是为了解决这些问题而生。

## UITabBarController

UITabBarController，和 [UINavigationController](https://github.com/alflix/awesome-ios/blob/master/UIKit/UINavigation/UINavigation.md#%E6%A6%82%E8%BF%B0) 一样是容器类（Container），利用集合的方式对 ViewController 进行管理。一般的做法都是将多个(一般是4-6个之间) UINavigationController 作为 UITabBarController 的子控件, 添加到 UITabBarController 的 UITabBar上。大部分情况下，UITabBarController 都会作为 app 的 rootViewController：
...
而第二种情况在 viewController push 子视图的时候，在大部分的 UI 交互中，需要设置 hidesBottomBarWhenPushed = true, 第一种则不需要。
