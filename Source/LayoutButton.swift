...
class LayoutButton: UIButton {
    var bulgeOffsetY: CGFloat = 0

    /// 图片和文字的间距，默认为8
    @IBInspectable var imageTitleSpace: CGFloat = 8 {
        didSet { layoutSubviews() }
    }

    @IBInspectable var titleHeight: CGFloat = 12 {
        didSet { layoutSubviews() }
    }

    /// 图片相对文字的位置，默认为左
    @IBInspectable var imageDirectionInt: Int {
        get {
    ...
    var imageDirection: TitleImageDirection = .left {
        didSet { layoutSubviews() }
    }
